...
const resultCopy:Record<Language,Record<string,string>>={
  ES:{demo:"DATOS TEMPORALES DE PRUEBA",synthesis:"Síntesis",guidance:"Orientación",method:"Método y alcance",questionOptional:"Pregunta o intención (opcional por ahora)",calculating:"Calculando mapa completo…",calculationError:"No fue posible completar el cálculo. Revisa los datos e inténtalo nuevamente.",retry:"Intentar de nuevo"},
  EN:{demo:"TEMPORARY TEST DATA",synthesis:"Synthesis",guidance:"Guidance",method:"Method and scope",questionOptional:"Question or intention (optional for now)",calculating:"Calculating full chart…",calculationError:"The calculation could not be completed. Check the data and try again.",retry:"Try again"},
  FR:{demo:"DONNÉES DE TEST TEMPORAIRES",synthesis:"Synthèse",guidance:"Orientation",method:"Méthode et portée",questionOptional:"Question ou intention (facultative pour l’instant)",calculating:"Calcul complet de la carte…",calculationError:"Le calcul n’a pas pu être terminé. Vérifiez les données et réessayez.",retry:"Réessayer"},
  DE:{demo:"TEMPORÄRE TESTDATEN",synthesis:"Synthese",guidance:"Orientierung",method:"Methode und Umfang",questionOptional:"Frage oder Absicht (vorerst optional)",calculating:"Vollständige Karte wird berechnet…",calculationError:"Die Berechnung konnte nicht abgeschlossen werden. Prüfen Sie die Daten und versuchen Sie es erneut.",retry:"Erneut versuchen"},
  PT:{demo:"DADOS TEMPORÁRIOS DE TESTE",synthesis:"Síntese",guidance:"Orientação",method:"Método e escopo",questionOptional:"Pergunta ou intenção (opcional por enquanto)",calculating:"Calculando o mapa completo…",calculationError:"Não foi possível concluir o cálculo. Confira os dados e tente novamente.",retry:"Tentar novamente"},
};
...
export function AstroConsultationFlow({ discipline, focus, focusIndex, lang, fromProfile=false }: { discipline: AstroDiscipline; focus: string; focusIndex: number; lang: Language; fromProfile?: boolean }) {
  ...
  useEffect(()=>{let active=true;setSubmitted(null);setCalculation(null);setError("");if(!fromProfile){setLoading(false);window.setTimeout(()=>document.querySelector(".astro-consultation-flow")?.scrollIntoView({behavior:"smooth",block:"start"}),50);return()=>{active=false}}const payload=buildPayload(discipline,focus,lang,initial);setLoading(true);void calculateAllAstros(payload).then(calculated=>{if(!active)return;setCalculation(calculated);setSubmitted(payload);window.setTimeout(()=>document.querySelector(".astro-result")?.scrollIntoView({behavior:"smooth",block:"start"}),50)}).catch(cause=>{if(!active)return;console.error("Astros calculation failed",cause);setError(t.calculationError)}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[discipline,focus,lang,fromProfile,retry]);
