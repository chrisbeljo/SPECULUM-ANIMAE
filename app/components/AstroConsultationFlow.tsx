...
export function AstroConsultationFlow({ discipline, focus, focusIndex, lang, fromProfile=false,onOpenProfile,onOpenAccount }: { discipline: AstroDiscipline; focus: string; focusIndex: number; lang: Language; fromProfile?: boolean;onOpenProfile?:()=>void;onOpenAccount?:()=>void }) {
  ...
  const {interpretation:aiInterpretation,isLoading:aiLoading}=useAstroInterpretation(submitted,focusIndex,calculation);
  if(submitted&&calculation&&aiInterpretation){const interpretation=aiInterpretation;return <section className={`astro-consultation-flow astro-result ${disciplineClass}`}>{!fromProfile&&<button className="astro-edit" onClick={()=>{setSubmitted(null);setCalculation(null)}}>← {t.new}</button>}<div className="astro-result-hero"><img src={interpretation.image} alt=""/><div><small>{user?t.profile:t.result}</small><h2>{interpretation.title}</h2><p><b>{t.focus}:</b> {focus}</p></div></div><AstroInterpretationChart discipline={discipline} focusIndex={focusIndex} lang={lang} data={calculation}/><div className="astro-result-grid"><article><small>{t.data}</small><h3>{submitted.birthName||submitted.name}</h3><p>{submitted.birthDate}{submitted.birthTime?` · ${submitted.birthTime}`:""}</p><p>{submitted.birthPlace}</p>{submitted.question&&<p>{submitted.question}</p>}</article><article><small>{t.calculation}</small><div className="astro-key-grid">{interpretation.keys.map(item=><p key={item.label}><span>{item.label}</span><b>{item.value}</b></p>)}</div></article><article className="astro-reading">{aiLoading&&<p className="astro-ai-loading">{t.enrichingReading}</p>}<small>{t.synthesis}</small><p>{interpretation.summary}</p>{interpretation.sections?.map(section=><div className="astro-reading-section" key={section.title}><h4>{section.title}</h4><p>{section.text}</p></div>)}</article><article className="astro-reading"><small>{t.guidance}</small><p>{interpretation.guidance}</p></article><article className="astro-interpretation-contract"><small>{t.method}</small><p>{interpretation.method}</p></article></div><AstroCalculationDetails discipline={discipline} focusIndex={focusIndex} lang={lang} data={calculation}/></section>}
  if(fromProfile&&!profileLoading&&user&&!isAstroProfileComplete(profile))return <section className={`astro-consultation-flow astro-profile-calculation ${disciplineClass}`}><header><small>{t.profile}</small><h2>{t.profileRequired}</h2><button type="button" className="astro-submit" onClick={onOpenProfile}>{t.completeProfile}</button></header></section>;
  if(fromProfile&&!profileLoading&&!user)return <section className={`astro-consultation-flow astro-profile-calculation ${disciplineClass}`}><header><small>{t.profile}</small><h2>{t.accountRequired}</h2><button type="button" className="astro-submit" onClick={onOpenAccount}>{t.openAccount}</button></header></section>;
  if(fromProfile)return <section className={`astro-consultation-flow astro-profile-calculation ${disciplineClass}`} aria-live="polite"><header><small>{t.demo}</small><h2>{loading?t.calculating:t.calculationError}</h2>{error&&<><p className="astro-form-error">{error}</p><button type="button" className="astro-submit" onClick={()=>setRetry(value=>value+1)}>{t.retry}</button></>}</header></section>;
