...
export function useUserProfile(){
  ...
  useEffect(()=>{void load();const refresh=()=>void load();window.addEventListener("speculum-profile-updated",refresh);return()=>window.removeEventListener("speculum-profile-updated",refresh)},[load]);

  const save=async(next:UserProfileData)=>{if(!user)throw new Error("No hay una sesión activa");const stored={...next,updatedAt:new Date().toISOString()};const {error:saveError}=await supabase.from("users").upsert({id:user.id,email:user.email,nombre:stored.fullName,perfil_datos:stored},{onConflict:"id"});if(saveError)throw saveError;setProfile(stored);window.dispatchEvent(new CustomEvent("speculum-profile-updated"));return stored};
