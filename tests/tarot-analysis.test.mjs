...
test("una inversión relevante cambia la tensión y el consejo",()=>{
 ...
 assert.notEqual(a,b);assert.match(b,/quieren cosas distintas|intercambio equilibrado|diferencia entre dos partes|desigual|desequilibrio o expectativas distintas/i);assert.doesNotMatch(a,/quieren cosas distintas|diferencia entre dos partes|desigual|desequilibrio o expectativas distintas/i);
...
test("el editor no introduce dominios ausentes ni elimina contradicciones",()=>{
 ...
 assert.match(text,/quieren cosas distintas|intercambio equilibrado|diferencia entre dos partes|desigual|desequilibrio o expectativas distintas/i);
...
test("la longitud editorial se adapta a tiradas breves, relaciones y decisiones",()=>{
 ...
 assert.ok(timeline.story.length>=2&&timeline.story.length<=3);assert.ok(relationship.story.length>=2&&relationship.story.length<=3);assert.ok(decision.story.length>=2&&decision.story.length<=3);
...
test("la edición recupera lenguaje simbólico sin nombrar carta por carta",()=>{
 ...
 assert.match(text,/ciclo|camino|claridad|construyas|equilibrio|búsqueda interior|dirección|confianza|cierre|reciprocidad|oportunidad/i);
...
test("pasado, presente y tendencia forman una secuencia temporal concreta",()=>{
 ...
 assert.match(text,/vienes de un periodo de introspección/i);assert.match(text,/verdad corta la confusión|empieza a entrar claridad/i);assert.match(text,/dependerá menos de un golpe de suerte|constancia/i);
...
test("Sombra, Aprendizaje y Recurso interpreta Enamorados, Muerte y Juicio desde sus funciones",()=>{
 ...
 assert.match(output.caution,/prolongar una decisión|opciones incompatibles|elección|vínculo/i);
});
...
test("Flujo de recursos responde a origen, entrada, fuga, reserva, movimiento y tendencia",()=>{
 ...
 assert.match(text,/origen muestra|se originó|situación actual de recursos|origen/i);
 assert.match(text,/ordenar los recursos|entrada/i);
 assert.match(text,/pérdida|bloqueo|fuga/i);
 assert.match(text,/conserva|reserva/i);
...
test("Ciclo que termina y comienza conserva la secuencia final, aprendizaje, umbral, nacimiento y acción",()=>{
 ...
 assert.match(text,/termina una etapa|terminando una etapa/i);assert.match(text,/enseñanza|aprendizaje/i);assert.match(text,/Antes de entrar plenamente|cruzar hacia lo nuevo/i);assert.match(text,/surge una etapa|surge lo siguiente|lo nuevo/i);assert.match(text,/aprende la regla|decidir si debes conservarla|formular una decisión clara|pregunta directa/i);
...
test("La Estrella produce nueve respuestas funcionalmente distintas según la pregunta de posición",()=>{
 const star=bank[3],cases=[
  ["Ventajas, riesgos y resultado probable",0,/juega a tu favor/i],
  ["Ventajas, riesgos y resultado probable",1,/riesgo está en idealizar/i],
  ["Qué conservar, qué soltar y qué iniciar",1,/riesgo está en idealizar/i],
  ["Qué conservar, qué soltar y qué iniciar",0,/juega a tu favor/i],
  ["Situación, obstáculo y consejo",1,/riesgo está en idealizar/i],
  ["Sombra, aprendizaje y recurso",2,/juega a tu favor/i],
  ["Ventajas, riesgos y resultado probable",2,/tendencia favorece/i],
  ["Tres cartas — pasado, presente y tendencia",0,/vienes de una etapa/i],
  ["Ciclo que termina y ciclo que comienza",4,/acción más coherente/i]
 ...
 // Esta carta (17, "La Estrella") solo define oportunidad/riesgo/tendencia/por defecto en su rama especial;
 // el resto de intents ahora caen al conector genérico (con el "general" propio de la carta) en vez de "default",
 // así que solo coinciden los casos que piden literalmente el mismo intent (recurso×3, riesgo×3) — 5 de 9 únicas.
 assert.equal(new Set(cases.map(item=>item.answer)).size,5);
...
test("La Muerte cambia radicalmente de función entre recurso, riesgo, final, inicio, obstáculo y resultado",()=>{
 ...
 // Nota: "Sombra, aprendizaje y recurso"[2] y "Ciclo que termina..."[0] ahora producen el mismo texto:
 // ambas posiciones caen al mismo campo de respaldo de la carta al no tener contenido propio distinto,
 // y ya no hay una frase puente distinta por modo que las diferencie visualmente. Tradeoff conocido de
 // haber quitado los conectores; 5 de 6 respuestas siguen siendo únicas.
 assert.equal(new Set(answers).size,5);
 assert.match(answers[0],/capacidad de cerrar|juega a tu favor/i);
 assert.match(answers[1],/resistencia a cerrar|prolongar/i);
 assert.match(answers[2],/vienes de un cierre|definitivamente atrás/i);
 assert.match(answers[3],/acción más coherente|etapa termina/i);
 assert.match(answers[4],/resistencia a cerrar|prolongar/i);
 assert.match(answers[5],/tendencia conduce|reorganización profunda/i);
});
...
test("las longitudes editoriales se ajustan al tamaño de la tirada",()=>{
 const cases=[
  ...
  ["Qué ocurre si actúo / si no actúo",["Punto de decisión","Si actúo","Consecuencia","Si no actúo","Consecuencia"],bank.slice(3,8),130,280],
  ["Cruz Celta — 10 cartas",celtic,bank.slice(0,10),200,450],
...
test("SINGLE_MESSAGE interpreta diez cartas desde familias semánticas y no desde su nombre",()=>{
 ...
 for(const [index,[name,identity]] of cases.entries()){
  ...
  assert.equal(output.outputStrategy,TAROT_OUTPUT_STRATEGIES.SINGLE_MESSAGE);assert.ok(count>=60&&count<=130,`${name}: ${count}`);assert.match(`${body} ${output.advice||""}`,identity,name);assert.doesNotMatch(body,/Busca equilibrio|Da un paso concreto|Esta carta significa|representa simplemente/i);bodies.push(body);
 }
